...
#define FALSE 0
 ...
 || defined(vax) || defined (MIPSEL) || defined (__alpha) \
 ...
 * It assumes tmp is declared as uint32, so that the >>
...
void MDreverse __ARGS((uint32 *X));
...
MDptr MDp;
{
	...
	for(i=0;i<4;i++)
		for(j=0;j<32;j=j+8)
		  printf("%02lx",(unsigned long)(MDp->buffer[i]>>j) & 0xFF);
}
